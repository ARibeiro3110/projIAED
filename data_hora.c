...
/* Recebe duas datas e retorna TRUE caso estas sejam iguais, e FALSE caso
contrário. */
Bool datasIguais(Data d1, Data d2) {
	return d1.ano == d2.ano && d1.mes == d2.mes && d1.dia == d2.dia;
}

/* Recebe uma data e retorna TRUE caso esta seja anterior à data atual do
sistema, e FALSE caso contrário. */
Bool ehPassado(Data d) {
	return converteDataNum(d) < converteDataNum(_hoje);
}
